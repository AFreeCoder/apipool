<template>
  ...
  >
    ...
    >
      ...
      <!-- Kiro credentials -->
        ...
        <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
          ...
          <div v-if="poolModeEnabled" class="mt-3">
            <label class="input-label">{{ t('admin.accounts.poolModeRetryStatusCodes') }}</label>
            <input
              id="kiro-pool-mode-retry-status-codes"
              v-model="poolModeRetryStatusCodesInput"
              type="text"
              class="input"
              :placeholder="DEFAULT_POOL_MODE_RETRY_STATUS_CODES.join(', ')"
      ...
      <div
        v-if="supportsQuotaLimit(form.type) && form.platform === 'anthropic' && (form.type === 'apikey' || form.type === 'bedrock')"
      ...
      <div
        v-else-if="supportsQuotaLimit(form.type)"
        class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4"
      >
        <div class="mb-3">
          <h3 class="input-label mb-0 text-base font-semibold">{{ t('admin.accounts.quotaControl.title') }}</h3>
...
const accountCategory = ref<'oauth-based' | 'apikey' | 'bedrock' | 'kiro' | 'service_account'>('oauth-based') // UI selection for account category
