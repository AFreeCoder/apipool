<template>
  ...
  >
    <div class="space-y-4">
      ...
      <template v-else>
        <!-- Description -->
        <p v-if="platformDescription" class="text-sm text-gray-600 dark:text-gray-400">
          {{ platformDescription }}
        </p>
...
watch(
  () => [props.platform, props.apiKey, props.baseUrl],
  () => {
    resetOpenClawState()
  }
)
...
const openClawImportSpec = computed(() => {
  ...
  return buildOpenClawImportSpec(props.platform, baseUrl, props.apiKey)
})
...
const showPlatformNote = computed(() => activeClientTab.value !== 'opencode' && activeClientTab.value !== 'openclaw')
...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready APIPool setups.
  ...
  const configContent = `# Grok Build CLI → APIPool Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for APIPool Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches APIPool).
...
description = "Grok 4.5 via APIPool (Responses)"
...
# Imagine tools: model IDs go to APIPool media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set APIPOOL_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:APIPOOL_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export APIPOOL_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → APIPool Grok group
...
# Grok/APIPool path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:APIPOOL_API_KEY="${apiKey}"`
    : `export APIPOOL_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> APIPool ${label} group
model_provider = "apipool"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at APIPool OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via APIPool'
