...
// APIPool 的外部充值入口与上游内建支付开关独立。
describe('FeatureFlags.purchaseSubscription', () => {
  it('uses the external purchase setting independently of built-in payment', () => {
    expect(resolveFeatureFlag({ purchase_subscription_enabled: true, payment_enabled: false } as PublicSettings, FeatureFlags.purchaseSubscription)).toBe(true)
    expect(resolveFeatureFlag({ purchase_subscription_enabled: false, payment_enabled: true } as PublicSettings, FeatureFlags.purchaseSubscription)).toBe(false)
  })
})
