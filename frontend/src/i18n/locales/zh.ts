  ...
  admin: {
    ...
    accounts: {
      ...
      schedulerScore: {
        ...
        stickyShort: '粘性上限',
        ungrouped: '未分组',
        hint: '显示格式为“分组名 / 基础分 / 粘性上限分”。基础分按当前筛选条件限定的候选账号计算，包含优先级、负载、排队、错误率、首包延迟、重置窗口、额度余量等因子；粘性上限分表示请求命中 previous_response_id 或 session_hash 后可能达到的分数，不代表当前列表快照已命中粘性。分数越大越优先。'
      },
      ...
      quotaLimitHint: '设置日/周/总使用额度（美元），任一维度达到限额后账号暂停调度。Anthropic API Key 与 Kiro 账号还可配置客户端亲和。修改限额不会重置已用额度。',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 APIPool 实例的场景。',
      ...
      kiroAuthMethod: 'Kiro 认证方式',
      kiroSocialAuth: 'Social',
      kiroIDCAuth: 'IdC',
      kiroRefreshToken: 'Kiro refresh token',
      kiroClientId: 'Kiro client ID',
      kiroClientSecret: 'Kiro client secret',
      kiroAuthRegion: 'Kiro 鉴权区域',
      kiroApiRegion: 'Kiro API 区域',
      kiroMachineId: 'Kiro machine ID',
      kiroMachineIdHint: '留空则按 refresh token 自动生成',
      kiroProfileArn: 'Kiro profile ARN',
      kiroRefreshTokenRequired: 'Kiro refresh token 不能为空',
      kiroIDCClientRequired: 'Kiro IdC 需要 client ID 和 client secret',
      // Upstream type
      upstream: {
        baseUrl: '上游 Base URL',
    ...
    ops: {
      ...
      requestLog: {
        ...
        truncated: '是否截断',
        truncatedShort: '已截断',
        memoryPrecheck: 'Redis 内存预检：已用 {used} / 上限 {max}（{percent}%）',
        memoryGuarded: '内存保护已触发',
        retentionHint: '仅保留最近一小段请求；发生截断或丢弃时请缩小排查窗口或尽快下载。',
        complianceNotice: '将记录该用户完整请求与响应明文，仅用于排查',
        reasonPlaceholder: '请输入本次排查原因（必填）',
        confirmThirtyMinutes: '确认最长录制 30 分钟，到期自动停止',
        forceRestart: '如已在录制，强制结束旧会话并开启新会话',
        enabling: '开启中...',
        disabling: '关闭中...',
        enabledSuccess: '请求日志录制已开启',
        disabledSuccess: '请求日志录制已关闭',
        failedToLoad: '加载请求日志状态失败',
        failedToLoadItems: '加载请求日志条目失败',
        failedToLoadDetail: '加载请求日志详情失败',
        failedToEnable: '开启请求日志录制失败',
        failedToDisable: '关闭请求日志录制失败',
        failedToDownload: '创建下载链接失败',
        noItems: '当前会话暂无请求条目',
        detail: '详情',
      ...
      settings: {
        ...
        ignoredErrorCodes: '屏蔽错误码白名单',
        ignoredErrorCodesHint: '按精确错误码匹配，支持逗号、分号或换行分隔。仅影响后续错误日志写入，不修改历史数据。',
    ...
    settings: {
      ...
      apiKeyAcl: {
        ...
        trustForwardedIpHint:
          '默认关闭。开启前需配置 server.trusted_proxies，并确保反代覆盖 X-Forwarded-For / X-Real-IP；API Key IP 白/黑名单只会信任这些可信代理解析出的客户端 IP。'
      },
      linuxdo: {
        title: 'LinuxDo Connect 登录',
        description: '配置 LinuxDo Connect OAuth，用于 APIPool 用户登录',
