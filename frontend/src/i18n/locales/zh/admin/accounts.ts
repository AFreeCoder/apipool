export default {
    accounts: {
      kiroLabel: "Kiro",
      kiroDesc: "通过 Kiro OAuth 接入",
      kiroAuthMethod: "授权方式",
      kiroSocialAuth: "社交账号授权",
      kiroIDCAuth: "IAM Identity Center 授权",
      kiroClientId: "客户端 ID",
      kiroClientSecret: "客户端密钥",
      kiroRefreshToken: "刷新令牌",
      kiroRefreshTokenRequired: "请输入刷新令牌",
      kiroIDCClientRequired: "请输入客户端 ID 和密钥",
      kiroApiRegion: "API 区域",
      kiroAuthRegion: "授权区域",
      kiroProfileArn: "配置 ARN",
      kiroMachineId: "设备 ID",
      kiroMachineIdHint: "可选，使用已有设备标识",
      ...
      schedulerScore: {
        ...
        stickyShort: '粘性上限',
        ungrouped: '未分组',
        hint: '显示格式为“分组名 / 基础分 / 粘性上限分”。基础分按当前筛选条件限定的候选账号计算，包含优先级、负载、排队、错误率、首包延迟、重置窗口、额度余量、计费倍率等因子；粘性上限分表示请求命中 previous_response_id 或 session_hash 后可能达到的分数，不代表当前列表快照已命中粘性。分数越大越优先。'
      },
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 APIPool 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 APIPool 端解除该限制。',
      ollamaCloud: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。APIPool 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 APIPool 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      grok: {
        ...
        ttsTextLabel: 'TTS 文本',
        ttsTextPlaceholder: '例如：Hello from APIPool connectivity test.',
      ...
      oauth: {
        kiro: {
          title: "Kiro 授权",
          provider: "登录提供商",
          providerHint: "选择 Kiro 账号使用的登录提供商",
          browserLogin: "在浏览器中登录",
          manualInput: "手动输入",
          importantNotice: "请使用与 Kiro 账号一致的登录方式",
          missingExchangeParams: "缺少授权交换参数",
          failedToExchangeCode: "授权码交换失败",
