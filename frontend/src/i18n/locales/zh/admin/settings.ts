export default {
    settings: {
      marquee: {
        ...
        textRequired: "请输入公告内容",
        tipMultipleSeparator: "可配置多条公告并调整顺序",
      },
      ...
      features: {
        ...
        siteBillingMode: {
          title: '站点类型',
          description: '控制内建购买方式和相关入口显示，默认「充值 & 订阅」。外部嵌入充值页的内容与可购买商品仍由外部系统控制。',
          label: '购买方式',
          options: {
          ...
          hints: {
            rechargeAndSubscription: '内建购买页同时提供余额充值与订阅套餐。',
            rechargeOnly: '用户端隐藏「我的订阅」、内建购买页订阅套餐、顶栏订阅进度与用量页「计费类型」筛选，直接访问「我的订阅」会跳回仪表盘；管理端侧边栏同时隐藏「订阅管理」入口（页面仍可通过地址访问）。已有订阅的计费与兑换码发放的订阅不受影响。',
            subscriptionOnly: '内建购买页只保留订阅套餐，侧边栏入口显示为「订阅」，内建余额充值下单会被拒绝；兑换码、返利等余额入账不受影响。',
          },
        },
        modelPlaza: {
          title: '模型广场',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 APIPool 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，APIPool 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 APIPool 订单中，上游 type 会作为易支付 type 参数提交。',
