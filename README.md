...
- **无商业授权**：本项目未授权任何个人或组织基于本项目开展商业化运营。任何以本项目名义或基于本项目从事的商业行为均与本项目及开发者无关，相关纠纷、损失和法律责任由行为主体自行承担。
...
长耗时的 OpenAI / Grok 图片生成与编辑可通过 `/v1/images/generations/async` 或 `/v1/images/edits/async` 提交，再通过 `/v1/images/tasks/{task_id}` 轮询结果，避免持续占用 CDN 长连接。请求和响应示例见 [docs/ASYNC_IMAGE_TASKS.md](docs/ASYNC_IMAGE_TASKS.md)。

## Grok / xAI 支持

APIPool 同时支持通过 xAI OAuth 接入 Grok 订阅账号，以及使用标准 xAI API Key 的账号。两类账号都可承接 OpenAI-compatible 请求，并按账号类型选择订阅代理或 xAI API 上游。
...
- OpenAI-compatible Images：`/v1/images/generations`、`/images/generations`、`/v1/images/edits`、`/images/edits`
- 异步图片任务：`/v1/images/generations/async`、`/v1/images/edits/async`、`/v1/images/tasks/{task_id}`
- OpenAI-compatible Videos：`/v1/videos/generations`、`/videos/generations`、`/v1/videos/edits`、`/videos/edits`、`/v1/videos/extensions`、`/videos/extensions`、`/v1/videos/{request_id}`、`/videos/{request_id}`
...
- 媒体生成请求要求分组开启图片生成权限
- 暂不支持：TTS、转录、浏览器自动化、cookie 和 Grok 网页抓取
...
| `XAI_BASE_URL` | `https://api.x.ai/v1`；用于运行时诊断，实际请求转发以账号 `base_url` 为准 |

管理员可以在后台创建 Grok OAuth 或 API Key 账号，也可以通过以下管理 API 完成 OAuth 授权或重新授权：
...
xAI quota 是被动展示：系统不会自造订阅额度，只会在 xAI 返回 rate-limit headers 时记录并展示。OAuth 推理默认走 `https://cli-chat-proxy.grok.com/v1`，显式配置的自定义上游保持不变；API Key 账号默认走 `https://api.x.ai/v1`。`401` 会标记账号凭据无效并临时移出调度；`403` 作为权益或订阅层级失败处理；`429` 根据 `Retry-After` 或短 cooldown 临时移出调度。
