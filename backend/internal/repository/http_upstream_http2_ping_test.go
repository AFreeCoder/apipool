		...
		t.Run(tc.name, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				if tr.Protocols != nil {
					...
					// Go 1.26 使用 x/net HTTP/2；直接复用生产保活配置器。
					h2, err := enableHTTP2KeepAlive(&http.Transport{}, tc.mode)
					require.NoError(t, err)
					h2.AllowHTTP = true
					h2.ConnPool = nil
					h2.DialTLSContext = func(context.Context, string, string, *tls.Config) (net.Conn, error) {
						return clientConn, nil
					}
					defer h2.CloseIdleConnections()
					transport = h2
				}
				client := &http.Client{Transport: transport, Timeout: 50 * time.Second}
				started := time.Now()
