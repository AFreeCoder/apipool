...
const DefaultCSPPolicy = "default-src 'self'; worker-src 'self' blob:; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://*.alicdn.com https://static.cloudflareinsights.com https://turing.captcha.qcloud.com https://turing.captcha.gtimg.com https://ca.turing.captcha.qcloud.com https://global.turing.captcha.gtimg.com https://www.tycaptcha.com https://cloudcache.tencentcs.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://*.captcha.gtimg.com https://fonts.googleapis.com https://*.alicdn.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: blob: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https://turing.captcha.qcloud.com https://www.tycaptcha.com https://rce.tencentrio.com https:; frame-src 'self' https://challenges.cloudflare.com https://turing.captcha.qcloud.com https://ca.turing.captcha.qcloud.com https://www.tycaptcha.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com https:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
...
type RateLimitConfig struct {
	...
	OpenAIOAuth403ChallengeCooldownMinutes int `mapstructure:"openai_oauth_403_challenge_cooldown_minutes"` // OpenAI OAuth 403 Cloudflare challenge 冷却(分钟)
}
...
func setDefaults() {
	...
	viper.SetDefault("webauthn.rp_display_name", "APIPool")
	...
	// Preserve current production behavior by requiring an explicit opt-in
	// before switching ops dashboard queries to pre-aggregated tables.
	...
	viper.SetDefault("rate_limit.openai_oauth_403_challenge_cooldown_minutes", 10)

	// Pricing - 从 model-price-repo 主分支同步模型定价和上下文窗口数据
