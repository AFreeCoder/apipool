...
func TestClassifyOpenAICompatibleNoAccountError_GrokGroupUsesGrokPlatform(t *testing.T) {
	c := newTestGinContextWithRequest()
	fd := &fakeDiagnoser{resp: service.ModelAvailabilityDiagnosis{HasAccountsInPool: true, HasModelSupport: false}}
	...
	cls := classifyOpenAICompatibleNoAccountErrorFromGin(c, fd, apiKey, "grok-4", "grok-4")
