...
func SetupRouter(
	...
	// 解析模式按请求快照：开启时使用 server.trusted_proxies 可信代理链，关闭时只用直连对端。
	...
	if frontendMiddleware, onSettingsUpdate, ok := web.EmbeddedFrontendMiddleware(settingService, refreshFrameOrigins); ok {
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, apiKeyService, subscriptionService, opsService, reqLogService, settingService, compositeResolver, cfg, redisClient)
...
func registerRoutes(
	...
	routes.RegisterGatewayRoutes(r, h, apiKeyAuth, apiKeyService, subscriptionService, opsService, reqLogService, settingService, compositeResolver, cfg)
