...
func RegisterGatewayRoutes(
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), reqLogCapture, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	rootRoute(http.MethodPost, "/messages/count_tokens", textBodyLimit, countTokensHandler)
	codexDirect := r.Group("/backend-api/codex")
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), reqLogCapture, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
