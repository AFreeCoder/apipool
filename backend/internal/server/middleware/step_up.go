...
// 敏感操作必须绑定 refresh token family；无会话 ID 的旧 token 不允许降级为用户级授权。
func StepUpSessionKey(c *gin.Context) string {
	return strings.TrimSpace(c.GetString(ContextKeySessionID))
}
...
func enforceStepUp(c *gin.Context, grantChecker stepUpGrantChecker, userReader stepUpUserReader, settings stepUpSettingReader) bool {
	...
	if sessionKey == "" {
		AbortWithError(c, 401, "STEP_UP_SESSION_REQUIRED",
			"This session predates per-session verification; please login again")
		return false
	}
