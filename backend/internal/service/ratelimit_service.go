...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	case 401:
		// Spark 影子无独立凭据，401 是母账号 token 问题。失效缓存、refresh_token 判断、
		// 永久禁用、临时不可调度都必须落到凭据 owner；非影子时 resolveCredentialAccount 返回自身。
		authAccount := account
		if resolved, rerr := resolveCredentialAccount(ctx, s.accountRepo, account); rerr == nil && resolved != nil {
			authAccount = resolved
		}
		if authAccount.Type == AccountTypeOAuth && authAccount.Platform == PlatformOpenAI {
			if msg := buildOpenAIOAuthHTTPErrorMessage(statusCode, responseBody); msg != "" {
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, headers http.Header, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	baseMsg := buildForbiddenErrorMessage(
		"Access forbidden (403):",
		upstreamMsg,
		responseBody,
		"account may be suspended or lack permissions",
	...
	if account.IsOpenAIOAuth() {
		if terminalMsg := buildOpenAIOAuthHTTPErrorMessage(http.StatusForbidden, responseBody); terminalMsg != "" {
			s.handleAuthError(ctx, account, terminalMsg)
			return true
		}
	}

	if s.openAI403CounterCache == nil {
		s.handleAuthError(ctx, account, baseMsg)
		return true
	}
