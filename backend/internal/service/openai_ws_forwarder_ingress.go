...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, requestedReasoningEffort *string) (*OpenAIForwardResult, error) {
		...
		for {
			...
			if eventType == "error" {
				...
				fatalSessionError := fallbackReason == "auth_failed" || fallbackReason == "upgrade_required" ||
					fallbackReason == "ws_unsupported" || fallbackReason == "ws_connection_limit_reached" ||
					fallbackReason == "previous_response_not_found" ||
					(fallbackReason == openAIWSFallbackReasonInvalidEncryptedContent && len(collectOpenAIEncryptedContentDigestsRaw(payload)) == 0)
				if !wroteDownstream && !turnHasFunctionCallOutput && fatalSessionError {
					lease.MarkBroken()
					status := coderws.StatusPolicyViolation
					switch fallbackReason {
					case "auth_failed", "event_error", "upstream_error_event":
						status = coderws.StatusInternalError
					case "ws_connection_limit_reached":
						status = coderws.StatusTryAgainLater
					}
					reason := strings.TrimSpace(errMsgRaw)
					if reason == "" {
						switch fallbackReason {
						case "invalid_encrypted_content":
							reason = "upstream rejected encrypted content"
						case "":
							reason = "upstream error event"
						default:
							reason = fallbackReason
						}
					}
					return nil, NewOpenAIWSClientCloseError(status, reason, errors.New(reason))
				}
			}
			...
			if eventType == "error" && turnHasFunctionCallOutput {
				if clientDisconnected {
					lease.MarkBroken()
				}
	...
	for {
		...
		if relayErr != nil {
			...
			finalErr := relayErr
			var closeErr *OpenAIWSClientCloseError
			if !errors.As(relayErr, &closeErr) {
