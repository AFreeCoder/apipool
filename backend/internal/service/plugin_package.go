...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	//nolint:gosec // G703: inspectArchive 已验证插件 ID，禁止路径分隔符；父目录来自服务配置。
	...
	//nolint:gosec // G703: 源文件由 CreateTemp 创建，目标由配置目录及已校验清单标识组成。
	if err := os.Rename(tempPath, artifactPath); err != nil {
		_ = os.RemoveAll(installPath) //nolint:gosec // G703: 仅清理本函数创建的受管安装目录。
		return nil, fmt.Errorf("保存插件包: %w", err)
	}
	committed = true
	//nolint:gosec // G703: 读取刚刚存入受管目录的归档，路径组成字段已通过清单校验。
	artifactData, err := os.ReadFile(artifactPath)
	if err != nil {
		_ = os.Remove(artifactPath)   //nolint:gosec // G703: 仅清理本函数保存的受管归档。
		_ = os.RemoveAll(installPath) //nolint:gosec // G703: 仅清理本函数创建的受管安装目录。
