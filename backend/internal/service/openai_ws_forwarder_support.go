...
func classifyOpenAIWSErrorEventFromRaw(codeRaw, errTypeRaw, msgRaw string) (string, bool) {
	...
	if strings.Contains(code, "account_deactivated") ||
		(strings.Contains(errType, "invalid_request") && strings.Contains(msg, "account") && strings.Contains(msg, "deactivated")) {
		return "auth_failed", false
	}
...
func openAIWSErrorHTTPStatusFromRaw(codeRaw, errTypeRaw string) int {
	...
	errType := strings.ToLower(strings.TrimSpace(errTypeRaw))
	switch {
	case strings.Contains(code, "account_deactivated"):
