...
func TestOpenAIGatewayServiceRecordUsage_WebSearchLogUsesBaseRateMultiplier(t *testing.T) {
	groupID := int64(15)
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:      "resp_web_search_peak_rate",
			Model:          "gpt-5.6-sol",
	...
	require.InDelta(t, 0.01, userRepo.lastAmount, 1e-12)
}
...
func TestOpenAIGatewayServiceRecordUsage_GPTImage2TokenBillingSwitchUsesUsageTokens(t *testing.T) {
	...
	require.InDelta(t, 0.00375, usageRepo.lastLog.InputCost, 1e-12)
	require.InDelta(t, 0.002, usageRepo.lastLog.ImageInputCost, 1e-12)
