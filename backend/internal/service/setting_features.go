...
// IsStepUpEnabled 检查可选的管理员权限变更 step-up 2FA 门控是否启用（默认关闭）。
// 固定挂载在导出、备份及存储配置等高风险路由上的门控不读取该开关，始终生效。
...
func (s *SettingService) ResolveAuthSourceGrantSettings(ctx context.Context, signupSource string, firstBind bool) (ProviderDefaultGrantSettings, bool, error) {
	...
	if quotas, err := s.GetDefaultPlatformQuotas(ctx); err == nil {
		...
		slog.Warn("[Setting] load default platform quotas failed while resolving auth source grant settings", "error", err)
	}
