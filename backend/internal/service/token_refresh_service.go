...
func NewTokenRefreshService(
	...
	anthropicRefresher := newCompositeOAuthRefreshExecutor(claudeRefresher, kiroRefresher)

	// 每个平台只注册一次，同一份注册表同时决定执行器与仓储候选范围。
	// Anthropic OAuth 与 Kiro 共用 anthropic 平台，因此按账号类型组合执行器。
	s.registrations = []tokenRefreshRegistration{
		{platform: PlatformAnthropic, refresher: anthropicRefresher, executor: anthropicRefresher},
...
func (s *TokenRefreshService) processRefreshContext(parent context.Context) {
	...
	s.maybeRunOpenAIPlanSync(ctx, nil)
...
func (s *TokenRefreshService) postRefreshStateSync(ctx context.Context, account *Account) {
	...
	// OpenAI OAuth: 刷新成功后同步 plan_type，再写入调度器缓存，确保缓存包含最新值。
