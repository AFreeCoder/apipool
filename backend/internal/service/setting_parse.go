...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		MarqueeMessages:                  s.getStringOrDefault(settings, SettingMarqueeMessages, "[]"),
...
func mergeProviderDefaultGrantSettings(globalDefaults ProviderDefaultGrantSettings, providerDefaults ProviderDefaultGrantSettings) ProviderDefaultGrantSettings {
	...
	if providerDefaults.PlatformQuotas != nil {
		...
		for platform, patch := range providerDefaults.PlatformQuotas {
			if result.PlatformQuotas[platform] == nil {
				result.PlatformQuotas[platform] = &DefaultPlatformQuotaSetting{}
			}
			mergePlatformQuotaDefaults(result.PlatformQuotas[platform], patch)
		}
	}
