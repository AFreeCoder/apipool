...
const (
	...
	defaultGrokTTSTestText       = "Hello from APIPool account connectivity test."
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		// 401 Unauthorized: 标记账号为永久错误。OpenAI OAuth 若已落了更精确的
		// 停用/403 状态，则避免用普通 401 文案覆盖。
		if resp.StatusCode == http.StatusUnauthorized && s.accountRepo != nil && (!isOAuth || strings.TrimSpace(account.ErrorMessage) == "") {
