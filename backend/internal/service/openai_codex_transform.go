...
func applyCodexOAuthTransform(reqBody map[string]any, isCodexCLI bool, isCompact bool) codexTransformResult {
	...
	// 先把 role:"tool" 这类 Responses 不接受的消息形态转换成 transcript item，
	// 再执行 APIPool 的 OAuth input 归一化，避免工具输出被包进 message content。
...
func isKnownCodexOAuthTopLevelItemType(itemType string) bool {
	...
	switch itemType {
	case "reasoning", "function_call_output", "item_reference", "tool_call", "function_call", "image_generation_call", "web_search_call":
		return true
	default:
		return false
	}
...
func filterCodexInput(input []any, preserveReferences bool) []any {
	...
	for _, item := range input {
		...
		if shouldPreserveCodexOAuthInputItem(m) && !isCodexToolCallItemType(typ) {
			if codexTopLevelItemShouldDropCallID(typ) {
				newItem := make(map[string]any, len(m))
				for key, value := range m {
					newItem[key] = value
				}
...
func codexTopLevelItemShouldDropCallID(typ string) bool {
