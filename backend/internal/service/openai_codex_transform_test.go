	...
	for _, placement := range []string{"top_level", "additional_tools"} {
		for _, mode := range []string{"auto", "required"} {
			t.Run(placement+"/"+mode, func(t *testing.T) {
				...
				input := []any{map[string]any{"type": "message", "role": "user", "content": []any{map[string]any{"type": "input_text", "text": "probe"}}}}
...
func TestApplyCodexOAuthTransform_JsonObjectKeepsJsonInstructionInInput(t *testing.T) {
	...
	require.Equal(t, "developer", developer["role"])
	require.Equal(t, []any{
		map[string]any{"type": "input_text", "text": "You are an assistant. Output JSON only."},
	}, developer["content"])
	user, ok := input[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "user", user["role"])
