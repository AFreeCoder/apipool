...
func TestExecuteRefundRejectsPendingRefundOrder(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
...
func TestQueryAndFinalizeRefundReturnsConflictWhenPendingWasAlreadyClaimed(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	order := createPendingRefundOrderForTest(t, ctx, client, "query-finalize-claimed")
	...
	restore := replacePaymentProviderFactoryForTest(t, &refundQueryProviderTestDouble{
		queryFn: func(ctx context.Context, req payment.RefundQueryRequest) (*payment.RefundResponse, error) {
