...
func TestOpenAIFirstOutputStageUnlinkFailurePermanentlyFallsBackToMemoryAndRetriesCleanup(t *testing.T) {
	...
	stage.createTemp = func() (*os.File, error) {
		createCalls++
		file, err := os.CreateTemp(tempDir, "sub2api-openai-first-output-fallback-*")
		if err == nil {
			createdTempPath = file.Name()
		}
		return file, err
	}
	removeCalls := 0
	stage.removeFile = func(path string) error {
		...
		if path != createdTempPath {
			return fmt.Errorf("拒绝删除测试临时目录之外的路径: %s", path)
		}
		return os.Remove(createdTempPath)
	}
