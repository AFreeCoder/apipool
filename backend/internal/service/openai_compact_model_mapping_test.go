...
func TestOpenAIGatewayService_Forward_GlobalCompactModelFallbackOverridesUpstreamModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_125","status":"completed","model":"gpt-5.4","output":[],"usage":{"input_tokens":1,"output_tokens":1}}`)),
	}}
...
func TestOpenAIGatewayService_OAuthPassthrough_GlobalCompactModelFallbackOverridesUpstreamModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.5","stream":true,"store":true,"instructions":"compact-pass","input":[{"type":"text","text":"compact me"}]}`)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid-compact-pass-fallback"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"cmp_125","model":"gpt-5.4","usage":{"input_tokens":2,"output_tokens":3}}`)),
	}}
	...
	require.Equal(t, "gpt-5.4", result.UpstreamModel)
	require.Equal(t, "gpt-5.4", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "gpt-5.5", gjson.GetBytes(rec.Body.Bytes(), "model").String())
}
