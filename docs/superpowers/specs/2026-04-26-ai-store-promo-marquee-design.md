...
- 文案可能较长，不能截断（→ 跑马灯横向滚动）
- 仅桌面端显示，移动端通过 CSS `hidden` 隐藏且不参与布局（DOM 仍存在；AppHeader 在 `<lg` 时已经很挤）
...
└──────────────┬──────────────────────┘        │
               ...
               ▼                                ▼
┌─────────────────────────────────────────────────────────────────┐
│             appStore.cachedPublicSettings (public)               │
│             SettingsView 本地 form (admin edit)                  │
└──────────────┬──────────────────────────────────────────────────┘
               │ computed: marqueeText = enabled msgs joined by " · "
               ▼
┌─────────────────────────────────────────────────────────────────┐
│  AppHeader.vue (中间槽位)                                        │
│   └─ <AppHeaderMarquee />     // 常驻插入；组件内部按数据条件渲染  │
│         · 桌面端: 渲染滚动 track 或保留 flex-1 空位               │
│         · 移动端: CSS hidden, 不参与布局                          │
...
| 层 | 数据形态 | 职责 |
|---|---|---|
| **service**（`SystemSettings` / `PublicSettings`） | `MarqueeMessages string`（与 `CustomMenuItems` 一致——`map[string]string` 直接读出来） | 透传；不做结构化校验 |
| **service**（`PublicSettingsInjectionPayload`） | `MarqueeMessages json.RawMessage` | 透传 raw JSON 给前端 SSR 注入；helper 保证输出合法 JSON 数组（空数组用 `"[]"`） |
| **admin handler**（PUT） | 接收 `[]dto.MarqueeMessage` | 全部校验（长度、trim、ID 格式/补齐/唯一性、sort_order 归一化）→ marshal 成 JSON 字符串 → 写入 service |
| **admin handler**（GET，`GetSettings`） | 输出 `[]dto.MarqueeMessage` | parse raw JSON，原样返回（admin 看全部，含 disabled） |
| **public handler**（`GetPublicSettings`）/ **SSR injection**（`GetPublicSettingsForInjection`） | 输出 `[]dto.MarqueeMessage` | parse raw JSON；过滤 `enabled=false`；按 `sort_order` 升序 |

### 4.2 文件改动清单

| 文件 | 改动内容 |
|---|---|
| `backend/internal/handler/dto/settings.go` | 增 `MarqueeMessage` 结构；`SystemSettings`/`PublicSettings` 各加 2 字段；新增 helper：`ParseMarqueeMessages(raw string) []MarqueeMessage`（admin GET 用，原样返回）和 `ParsePublicMarqueeMessages(raw string) []MarqueeMessage`（公开侧 + SSR 用，过滤 disabled + 排序）。仿 `ParseCustomMenuItems` 落点 |
| `backend/internal/service/domain_constants.go` | 增 2 个 setting key 常量 `SettingMarqueeEnabled` / `SettingMarqueeMessages` |
| `backend/internal/service/setting_service.go` | (a) `SystemSettings` / `PublicSettings` 加 2 字段（`MarqueeMessages` 用 `string`，与 `CustomMenuItems` 同款，`GetSystemSettings` 第 525 行附近映射 `settings[SettingMarqueeMessages]`）；(b) **`PublicSettingsInjectionPayload` 同步加 2 字段（`MarqueeMessages` 用 `json.RawMessage`）**——这是必须项，否则契约测试 `TestPublicSettingsInjectionPayload_SchemaDoesNotDrift` 会 fail；(c) `GetPublicSettingsForInjection` 映射这两个字段；(d) `InitializeDefaultSettings`（第 1681 行）默认值加 `SettingMarqueeMessages: "[]"` 与 `SettingMarqueeEnabled: "false"` |
| `backend/internal/handler/admin/setting_handler.go` | (a) `GetSettings`（第 79 行）调用 `dto.ParseMarqueeMessages` 把 raw JSON parse 成 `[]MarqueeMessage`；(b) `UpdateSettings`（PUT，校验段参考第 892-967 行 `custom_menu_items` 实现）增加完整 marquee 校验：长度上限 20、text trim+1-500、ID 格式 `^[a-zA-Z0-9_-]+$`、ID 长度 ≤32、ID 自动补齐（复用 `generateMenuItemID()` 或新写 `generateMarqueeMessageID()`）、ID 唯一性、`sort_order` 归一化为 `0..N-1`、marshal 成 JSON 字符串写入 `updates`；(c) 变更日志 diff（第 1831 行附近）加 `marquee_enabled` / `marquee_messages` 比较 |
| `backend/internal/handler/setting_handler.go` | `GetPublicSettings`（第 27 行）调用 `dto.ParsePublicMarqueeMessages`，输出 parsed + filtered + sorted 数组 |
| `backend/internal/server/api_contract_test.go` | 契约测试加新字段断言（admin GET/PUT、public GET） |
| `backend/internal/service/setting_service_test.go` | 单元测试覆盖：`InitializeDefaultSettings` 含两个 marquee 默认值；`GetSystemSettings` 字段映射 |
| `backend/internal/handler/admin/setting_handler_test.go` | 校验单测：长度、trim、ID 格式/补齐/唯一性、sort_order 归一化 |
| `backend/internal/handler/dto/settings_test.go`（如已存在）/新建 | `ParseMarqueeMessages` / `ParsePublicMarqueeMessages` 行为：空字符串容错、非法 JSON 容错、过滤 disabled、排序正确 |

**注**：项目中的 `backend/internal/service/settings_view.go` 主要承载 service-side 类型定义，**不是公开 handler 构造点**。parse/filter/sort 落在 `dto` 层 helper 中（与上面 `ParseCustomMenuItems` 同处），由 admin handler 与 public handler 各自调用。
...
- 移动端 (`<lg`) 通过 `hidden lg:block` CSS 隐藏，不参与布局（DOM 仍存在）
...
跟随项目现有惯例：**展示数据走 `appStore.cachedPublicSettings`，admin 编辑数据走 `SettingsView.vue` 本地 form**。
...
- [stores/adminSettings.ts](../../../frontend/src/stores/adminSettings.ts)：**本期不动**。该 store 只是只读缓存（如 `customMenuItems = ref<CustomMenuItem[]>`），现有 `custom_menu_items` 的增删改也都不在这里发生；本期 marquee 也无需在 admin store 中加字段。后续若有其他视图需要读 admin-only marquee 时再加
- [views/admin/SettingsView.vue](../../../frontend/src/views/admin/SettingsView.vue)：marquee 编辑全部发生在本地 `form`（与现有 `form.custom_menu_items` 同处）：
  - `form.marquee_enabled: boolean`
  - `form.marquee_messages: MarqueeMessage[]`
  - 新增本地编辑函数（仿 `addCustomMenu` / `removeCustomMenu` / `moveMenuItem`，参见 SettingsView 第 5987-6008 行）：
    - `addMarqueeMessage()`：push 一条空字幕 `{ id: '', text: '', enabled: true, sort_order: form.marquee_messages.length }`
    - `removeMarqueeMessage(index)`：splice + 重新分配 `sort_order`
    - `moveMarqueeMessage(index, direction)`：本地交换 + 重新分配 `sort_order`
  - 保存时 `submitSettings`（第 6362 行附近）payload 加 `marquee_enabled` / `marquee_messages`
- [api/admin/settings.ts](../../../frontend/src/api/admin/settings.ts)：DTO 类型同步加新字段
...
- **排序：选用上下箭头按钮**（不引入拖拽）。理由：
  - 与同页面 `custom_menu_items` 的现有实现完全一致（[SettingsView.vue:3683-3719](../../../frontend/src/views/admin/SettingsView.vue#L3683-L3719) 用上下箭头 + `moveMenuItem`）
  - 字幕条数 ≤20，箭头操作完全够用
  - 不需要引入或复用 `vue-draggable-plus`（虽然它在 package.json 中已有，但本页未用过）
- **编辑**：inline editing（textarea 直接挂在卡片里），跟现有"自定义菜单"风格一致
- **保存**：跟整个 SettingsView 的"保存"按钮共用（项目现有约定）
- **行内删除**：与现有列表交互一致（无需确认弹窗，可通过"启用"开关软删）
...
| 类型 | 文件 | 用例 |
|---|---|---|
| 后端单测 | `setting_service_test.go` | `InitializeDefaultSettings` 含 `SettingMarqueeMessages: "[]"`；JSON 序列化往返 |
...
| 前端 store | `app.spec.ts` / `adminSettings.spec.ts` | public fallback 默认值、注入配置归一化、admin 只读缓存映射正确 |
| 前端 view | `SettingsView.spec.ts` | 本地 form 新增/删除/上下移动字幕生效；保存 payload 形态符合 admin handler 期望；空白字幕保存前拦截 |
...
1. **本期排序使用上下箭头按钮**——与同页面 `custom_menu_items` 的现有交互保持一致，不引入新的拖拽交互和额外实现风险
...
## 13. 二次评审记录（Codex，2026-04-26）

### 13.1 评审结论

Claude 已经把第 12 节中的主要评审意见吸收到主设计里，整体方向比初版更扎实，可以继续推进到实施计划。但当前文档仍有几处实现落点和措辞与现有代码不完全一致，建议在编排实施计划前修正，避免开发时找错层级或引入不必要的状态管理。

**处理状态（2026-04-26 更新）**：5 条评审意见全部经源码核对验证为正确，已在主章节修订：

| 评审项 | 修订位置 |
|---|---|
| 13.2 P1 service 层类型 | §4.1 拆分为 4 行：service 用 `string`，仅 `PublicSettingsInjectionPayload` 用 `json.RawMessage` |
| 13.3 P1 admin 编辑落点 | §5.4 重写：`adminSettingsStore` 不动；编辑函数落到 `SettingsView.vue` 本地 form |
| 13.4 P2 移动端措辞统一 | §1.4 / §2 架构图 / §5.1 全部统一为"CSS 隐藏，不参与布局，DOM 仍存在" |
| 13.5 P2 函数名校正 | §4.2 全部用真名：`InitializeDefaultSettings` / `GetSettings` / `GetPublicSettings`；说明 parse helper 落在 `dto` 层而非 `settings_view.go` |
| 13.6 P3 排序方案 | §6.1 / §6.2 明确选择"上下箭头按钮"（与同页面 custom_menu_items 一致），不引入 vue-draggable-plus |

### 13.2 P1：service 层 raw JSON 类型描述不准确

当前文档在三层数据形态中写到：

> **service**（`SystemSettings` / `PublicSettings` / `PublicSettingsInjectionPayload`）使用 `MarqueeMessages json.RawMessage`

这和现有 `custom_menu_items` 范式不完全一致。当前代码中：

- `service.SystemSettings.CustomMenuItems` 是 `string`
- `service.PublicSettings.CustomMenuItems` 是 `string`
- 只有 `PublicSettingsInjectionPayload.CustomMenuItems` 使用 `json.RawMessage`

**建议修正**：

- `service.SystemSettings` / `service.PublicSettings` 使用 `MarqueeMessages string`
- `PublicSettingsInjectionPayload` 可使用 `json.RawMessage`，并通过安全 helper 输出合法 JSON 数组
- 文档中的“保留 raw JSON 字符串”应和具体类型保持一致，不要统一写成 `json.RawMessage`

### 13.3 P1：`adminSettingsStore` 的职责描述不符合现状

当前文档写：

> `stores/adminSettings.ts`：模仿现有 `customMenuItems` 的 add/update/delete/reorder 行为，新增一组 `marqueeMessages` 操作

但现有 `adminSettingsStore` 只缓存少量 admin 设置，例如 `customMenuItems`、ops 开关、payment 开关；它并没有 `customMenuItems` 的 add/update/delete/reorder 行为。自定义菜单的增删改排实际发生在 `SettingsView.vue` 的本地 `form.custom_menu_items` 上。

**建议修正**：

- `SettingsView.vue` 本地 form 增加 `marquee_enabled` / `marquee_messages`
- 在 `SettingsView.vue` 中实现 `addMarqueeMessage`、`removeMarqueeMessage`、`moveMarqueeMessage` 等本地编辑函数
- 保存时把 `form.marquee_enabled` / `form.marquee_messages` 放进 admin settings payload
- `adminSettingsStore` 只在确实有其他视图需要读取 admin-only marquee 配置时再加缓存字段；本期通常不需要新增编辑操作

### 13.4 P2：移动端“不渲染”和 CSS hidden 的说法仍需统一

文档前后仍有几处说法不完全一致：

- 高层架构图里写 `<AppHeaderMarquee v-if="lg && enabled && marqueeText" />`
- 业务约束和特性中仍写“移动端不渲染”
- 组件方案实际是始终插入 `<AppHeaderMarquee />`，由内部 `hidden lg:block` 做 CSS 隐藏

**建议修正**：

- 统一描述为“移动端 CSS 隐藏，不参与布局；DOM 可存在”
- 高层架构图改为 `<AppHeaderMarquee />`，并在组件内部根据 `marquee_enabled` / `marquee_messages` 渲染内容
- 删除或替换“移动端不渲染”这类容易误导测试实现的措辞

### 13.5 P2：部分函数名和代码位置引用不准确

当前文档中仍有几个函数名或代码区域不是现有代码里的真实名字：

- 早先文档中的默认设置函数名不是当前实现；现有默认设置初始化在 `InitializeDefaultSettings`
- `BuildAdminSettingsResponse` 不是现有函数名；admin GET 当前在 `GetSettings` 内直接构造 `dto.SystemSettings`
- `settings_view.go` 被描述成公开视图 parse/filter/sort 的主要落点，但当前它主要承载 service settings 类型定义，不是公开 handler 的构造点

**建议修正**：

- 默认值落点写成 `SettingService.InitializeDefaultSettings`
- admin GET 落点写成 `backend/internal/handler/admin/setting_handler.go` 的 `GetSettings`
- public GET 落点写成 `backend/internal/handler/setting_handler.go` 的 `GetPublicSettings`
- parse/filter/sort 逻辑建议放在 `dto.ParseMarqueeMessages` / `dto.ParsePublicMarqueeMessages` 这类 helper 中，再由 public handler 和 SSR injection 调用

### 13.6 P3：拖拽排序可以进一步明确复用方案

文档仍写“尽量复用项目现有拖拽组件；若无，回退上下箭头”。当前项目已经有 `vue-draggable-plus` 依赖，并在 `GroupsView.vue`、`PaymentProviderList.vue` 中使用。

**建议修正**：

- 如果本期愿意做拖拽，明确复用 `VueDraggable`，不要再写成“是否已有待确认”
- 如果为了降低风险选择上下箭头，也明确这是产品/实现取舍，而不是因为项目没有拖拽能力
- 这项不阻塞实施，但写清楚能减少计划阶段的反复确认

### 13.7 建议在进入实施计划前同步修订的条目

- §2 高层架构：去掉 `v-if="lg && enabled && marqueeText"`，改为组件常驻插入、内部内容条件渲染
- §4.1 三层数据形态：把 service 设置结构改为 `string`，仅 SSR 注入结构使用 `json.RawMessage`
- §4.2 文件改动清单：把默认值函数名改为 `InitializeDefaultSettings`，把 admin GET 位置改为 `GetSettings`
- §5.4 store 改动：把 marquee 编辑操作从 `adminSettingsStore` 调整到 `SettingsView.vue` 本地 form
- §7 测试计划：补充 `SettingsView` 本地 form 行为测试；删除“adminSettingsStore add/update/delete/reorder”测试项，除非实施时真的新增 store 行为
